...
@rust(py=False)
def use_modulo(f: Annotated[Callable[[int], int], "Bound<'py, PyAny>"], i: int) -> int:  # type: ignore[empty-body]
    """
...
@rust()
def use_round_sign(f: Annotated[Callable[[float, bool], int], "Bound<'py, PyAny>"], x: float) -> int:  # type:ignore[empty-body]
    """
...
def test_function_type_errors() -> None:
    ...
    with pytest.raises(TypeError):
        rust(round_sign, 1)  # type: ignore[arg-type]
    with pytest.raises(TypeError):
        rust_py(round_sign, 1)  # type: ignore[arg-type]
    with pytest.raises(TypeError):
        rust(round_sign_py, 1)  # type: ignore[arg-type]
