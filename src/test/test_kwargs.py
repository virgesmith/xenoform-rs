...
@rust(py=False)
def f_rust(n: int, /, x: float, y: float = 2.7, *, b: bool = False) -> str:  # ty: ignore[empty-body]
...
def test_pos_kwargs() -> None:
    with pytest.raises(TypeError):
        f_rust(1)  # ty: ignore[missing-argument]
    ...
    with pytest.raises(TypeError):
        f_rust(n=1, x=3.1)  # ty: ignore[positional-only-parameter-as-kwarg]
    assert f_rust(1, 3.1, b=True) == "n=1 x=3.1 y=2.7 b=true"
    assert f_rust(1, b=True, x=2.7) == "n=1 x=2.7 y=2.7 b=true"
    with pytest.raises(TypeError):
        f_rust(1, 3.1, 2.7, True)  # ty: ignore[too-many-positional-arguments]
...
def varkwargs(**args: Any) -> Annotated[int, "usize"]:  # ty: ignore[empty-body]
    """
    Ok(match args {
        Some(dict) => dict.len(),
        None => 0
